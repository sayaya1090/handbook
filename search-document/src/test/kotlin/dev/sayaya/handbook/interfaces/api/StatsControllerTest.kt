...
class StatsControllerTest : BehaviorSpec({
    ...
    Given("타임라인 통계 API") {
        ...
        When("GET /workspace/{workspace}/stats/timeline을 호출하면") {
            Then("200 OK가 반환된다") {
                client.get()
                    .uri("/workspace/$workspace/stats/timeline?from=2026-01-01T00:00:00Z&to=2026-03-31T23:59:59Z&interval=1")
    ...
    Given("타입별 분포 API") {
        ...
        When("GET /workspace/{workspace}/stats/distribution을 호출하면") {
            Then("200 OK가 반환된다") {
                client.get()
                    .uri("/workspace/$workspace/stats/distribution")
                    .header("Accept", "application/json")
                    .exchange()
                    .expectStatus().isOk
            }
    ...
    Given("워크스페이스 집계 통계 API") {
        ...
        When("GET /workspace/{workspace}/stats를 호출하면") {
            Then("200 OK와 집계 통계가 반환된다") {
                client.get()
                    .uri("/workspace/$workspace/stats")
                    .header("Accept", "application/json")
                    .exchange()
                    .expectStatus().isOk
                    .expectBody()
    ...
    Given("품질 이슈 API") {
        ...
        When("GET /workspace/{workspace}/quality-issues를 호출하면") {
            Then("200 OK와 이슈 목록이 반환된다") {
                client.get()
                    .uri("/workspace/$workspace/quality-issues")
                    .header("Accept", "application/json")
                    .exchange()
                    .expectStatus().isOk
                    .expectBody()
    ...
    Given("에이전트 활동 API") {
        ...
        When("GET /workspace/{workspace}/agent-activity를 호출하면") {
            Then("200 OK와 활동 목록이 반환된다") {
                client.get()
                    .uri("/workspace/$workspace/agent-activity")
                    .header("Accept", "application/json")
                    .exchange()
                    .expectStatus().isOk
                    .expectBody()
