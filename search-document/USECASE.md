...
| **액터** | 사용자 (document-ui 경유) |
| **선행조건** | 워크스페이스 접근 권한 보유, 문서 타입 선택 |
| **정상 흐름** | 1. 클라이언트가 `GET /workspace/{id}/documents/export`에 형식(format=csv 또는 json)과 필터 파라미터를 전송한다.<br>2. `ExportController`가 `DocumentSearchService.findAllForExport()`를 호출하여 필터 조건에 맞는 문서를 조회한다.<br>3. `CsvSerializer`(CSV) 또는 JSON 직렬화를 통해 응답 스트림으로 반환한다.<br>4. `Content-Disposition: attachment` 헤더로 파일 다운로드를 트리거한다. |
...
| **정상 흐름** | 1. 클라이언트가 `GET /workspace/{id}/{type}/{serial}/history`를 요청한다.<br>2. `DocumentSearchService.findHistory()`가 해당 문서의 모든 버전을 시간순으로 조회한다.<br>3. `Flux<Document>`로 전체 이력이 반환된다. |
