...
- 이 파일(`triage.md`) 수정 금지. `triage.notes.md` 만 갱신.
- 자가 갱신 트리거: (1) 같은 유형 증상을 3회 이상 triage 한 경우 — 패턴 기록, (2) plan 이
  실제 호출 후 `skip` 추가/제외 피드백을 받은 경우 — 판정 규칙 보정 후보 기록.
