...
1. **발급 주체**: 워크스페이스 관리자 또는 사용자 본인 (자신이 소속된 워크스페이스에 대해).
2. **제약 사항**:
   - 한 번 발급된 토큰의 비밀키는 재확인 불가 (최초 1회만 노출).
   - 토큰 발급 시 반드시 유효 기간(Expiry)을 설정해야 함 (최대 1년).
   - 토큰에 바인딩된 Role은 발급자의 권한을 초과할 수 없음.
3. **취소(Revoke)**: 유출 의심 시 즉시 취소 가능해야 하며, 취소 즉시 모든 Gateway 요청이 거부됨.
4. **인증 헤더**: `Authorization: Bearer <PAT_VALUE>` 형식을 사용함.
