...
편집 도구(캔버스 컨트롤러, 상태바 등)에서 사용하는 버튼은 명확한 시각적 계층과 일관된 조작감을 위해 다음 표준을 따른다.

| 속성 | 표준 | 비고 |
|------|------|------|
| **빌더 패턴** | `IconButtonElementBuilder.button().icon(...)` | 일관된 DOM 구조 보장 |
| **디자인 스타일** | Plain (배경 없음, 호버 시에만 표시) | MD3 Plain Icon Button 스펙 |
| **셰이프** | 원형 (`border-radius: 50%`) | 전용 편집 도구임을 시각적으로 구별 |
| **아이콘 스타일** | Font Awesome Sharp Solid | 액션의 명확성 확보 |

### 모바일 헤더 접근성

모바일 뷰포트에서 버튼이 많은 상태바나 툴바는 가로 방향으로 넘칠 수 있다. 이를 위해 다음 정책을 적용한다.

- **가로 스크롤**: `.type-status-header` 등 헤더 컨테이너에 `overflow-x: auto` 적용.
- **크기 유지**: 내부 버튼 및 요소에 `flex-shrink: 0`을 적용하여 찌그러짐 방지.
- **시각적 힌트**: 우측 끝에 미세한 그라데이션 mask를 적용하여 스크롤 가능함을 암시.
