...
- **Type-UI**: 캔버스에 핀치 줌, 터치 드래그 지원. 상단바를 제거하고 **멀티 플로팅 Speed Dial**(액션/설정) 및 **상단 플로팅 캡슐**(기간 정보) 패턴을 적용하여 작업 공간을 최대화함.
...
**역할:** Kafka 이벤트를 수신하여 워크스페이스별 SSE로 브로드캐스트한다. **실시간 협업의 중심 허브**로서, 같은 워크스페이스의 모든 참여자(사용자 + 에이전트)가 동일한 SSE 스트림(`/workspaces/{id}/messages`)을 구독한다. 사용자의 데이터 변경(DOCUMENT_CREATED, TYPE_CREATED 등)과 에이전트 커맨드(AGENT_COMMAND)가 모두 같은 Kafka 토픽("handbook-events")을 통해 동일한 SSE 스트림으로 전달되므로, 다른 사용자나 에이전트의 변경사항이 즉시 반영된다.
...
| 워크스페이스 SSE 스트림 공유 | 기존 `/workspaces/{id}/messages` SSE에서 AGENT_COMMAND 이벤트를 필터링하여 수신. 별도 SSE 연결 불필요 |
...
**에이전트는 "세 번째 협업자"** — 에이전트의 커맨드는 다른 도메인 이벤트(DOCUMENT_CREATED, TYPE_CREATED 등)와 동일한 Kafka 채널("handbook-events")을 통해 AGENT_COMMAND 타입으로 발행된다. event-broadcaster가 이를 워크스페이스별 SSE(`/workspaces/{id}/messages`)로 브로드캐스트하므로, 에이전트 전용 SSE 엔드포인트가 불필요하다. 워크스페이스의 모든 멤버가 에이전트의 작업 과정을 실시간으로 관찰할 수 있다.
...
| **type-command** | REST API | "GET /workspaces/{id}/types" | "PUT/DELETE /workspaces/{id}/types" | X (DB 직접 변경) |
| **workspace-command** | REST API | "(shell-ui WorkspaceList)" | "POST/PUT/DELETE /workspaces" | X (DB 직접 변경) |
| **type-query** | REST API | "GET /workspaces/{id}/types" | "읽기 전용" | - |
| **onboarding-ui** | REST API | - | "POST /workspaces (생성)" | X (DB 직접 변경) |
| **workspace-ui** | REST API | "GET /workspaces/{id}" | "PUT /workspaces/{id}, POST /workspaces/{id}/groups" | X (DB 직접 변경) |
...
| 컴포넌트 | 모듈 | 역할 |
|----------|------|------|
| `WorkspaceEventListener` | shell-ui | URI에서 워크스페이스 ID를 추출하고 `/workspaces/{id}/messages` SSE에 연결. 수신된 이벤트를 `handbook-workspace-event` CustomEvent로 window에 전파 |
...
sequenceDiagram
    ...
    UserA->>PD: "PUT /workspaces/{id}/documents"
