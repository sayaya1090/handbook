...
```
/auth/**, /user                     -> login
/workspaces/*/types/** (GET)         -> type-query
/workspaces/*/types/** (PUT/DELETE)  -> type-command
/workspaces/*/documents/** (GET)     -> document-query
/workspaces/*/documents/** (PUT/DEL) -> document-command
/workspaces/** (POST/PUT/DELETE)     -> workspace-command
/workspaces/*/messages (SSE)         -> event-broadcaster
...
```
command-*  ->  Kafka (handbook-events)  ->  event-broadcaster  ->  SSE  ->  Browser
```

사용자 변경과 AI 에이전트 커맨드가 동일한 SSE 스트림(`/workspaces/{id}/messages`)으로 통합 전달된다.
...
|--------------------|-----------------|-------------|-------------|
| `handbook-postgresql` | `classpath:postgresql.yaml` | `infrastructure/templates/cloudnative-pg/postgresql.yaml` | command-\*, query-\*, login |
| `handbook-kafka` | `classpath:kafka.yaml` | `infrastructure/templates/kafka/kafka.yaml` | command-\*, event-broadcaster, assistant |
| `handbook-authentication` | `classpath:authentication.yaml` | `infrastructure/templates/authentication/authentication.yaml` | gateway, event-broadcaster, command-\*, query-\* |
