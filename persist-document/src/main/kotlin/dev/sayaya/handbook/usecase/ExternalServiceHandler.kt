...
class ExternalServiceHandler(private val externals: List<ExternalService>) {
    fun publish(principal: Principal, workspace: UUID, documents: Map<ExternalService.DocumentKey, Document?>): Mono<ExternalPublishResult> = Flux.fromIterable(externals).flatMap {
        it.publish(principal, workspace, documents)
