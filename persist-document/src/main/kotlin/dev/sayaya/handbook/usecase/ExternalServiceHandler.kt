...
class ExternalServiceHandler(private val externals: List<ExternalService>) {
    fun publish(principal: Principal, workspace: UUID, types: Map<ExternalService.DocumentKey, Document?>): Mono<ExternalPublishResult> = Flux.fromIterable(externals).flatMap {
