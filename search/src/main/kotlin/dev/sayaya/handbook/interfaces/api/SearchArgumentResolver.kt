...
class SearchArgumentResolver : HandlerMethodArgumentResolver {
    ...
    override fun resolveArgument(
        ...
        val page = map.getFirst("page")?.toIntOrNull()
            ?: throw ResponseStatusException(HttpStatus.BAD_REQUEST, "Missing or invalid 'page' parameter")
        val limit = map.getFirst("limit")?.toIntOrNull()
            ?: throw ResponseStatusException(HttpStatus.BAD_REQUEST, "Missing or invalid 'limit' parameter")
        ...
        val filters = map.filterKeys { key -> key !in setOf("page", "limit", "sort_by", "asc") }
            ...
            .toMutableList()
        return Mono.just(Search(page, limit, sortBy, asc, filters))
    }
}
