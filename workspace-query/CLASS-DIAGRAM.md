...
못하도록 하는 안전장치. 실제 write 는 workspace-command 가 담당한다.

## 확장 지점

| 확장 | 추가 클래스 | 비고 |
|------|-------------|------|
| 워크스페이스 목록 조회 | `WorkspaceController` + `WorkspaceRepository` (read-only adapter) | `workspace-command` 의 R2DBC 레포지토리를 read-only 로 재사용 검토 |
