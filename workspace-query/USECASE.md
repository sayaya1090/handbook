...
- `GET /workspaces/{id}/groups` — 그룹 목록 (현재 workspace-command 에서만 조회)
- `GET /workspaces/{id}/permissions` — 권한 매트릭스 조회
- `operationId` 스네이크_케이스 통일 (현재 `list`/`get` → `list_workspaces`/`get_workspace`)

gateway 라우트 `workspace-query` (Path=`/workspaces,/workspaces/**`, Method=GET) 는 이미 등록되어 있다.
