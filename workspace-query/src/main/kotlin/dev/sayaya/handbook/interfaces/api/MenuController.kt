 ...
 * 추가되면 이 모듈의 별도 컨트롤러로 확장한다 — `workspace-command` 는 CUD 전용.
 */
@RestController
@Tag(
    name = "Workspace Menu",
...
class MenuController(private val repository: WorkspaceReadRepository) {
    companion object {
        val MENU: Menu = Menu.builder()
            ...
            .icon("fa-briefcase")
            .iconType("sharp")
            .script("/js/workspaces/workspace.nocache.js")
            .bottom(true)
            .tools(
                Tool.builder().title("workspace info").order("S1").icon("fa-information").iconType("sharp")
                    .url("/workspaces/{workspaceId}/info").urls("^/workspaces/\\{workspaceId\\}(/info)?$").build(),
                Tool.builder().title("groups").order("S5").icon("fa-users-gear").iconType("sharp")
                    .url("/workspaces/{workspaceId}/groups").urls("^/workspaces/\\{workspaceId\\}/groups$").build(),
