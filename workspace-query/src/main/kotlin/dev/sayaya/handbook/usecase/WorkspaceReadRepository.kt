 ...
 *
 * **주의:** workspace-command 의 `WorkspaceRepository` 와 의도적으로 분리됨.
 * workspace-query 는 read-only PostgreSQL 세션으로 연결되어, 구현체가 SELECT 만
...
interface WorkspaceReadRepository {
     ...
     * 어드민 그룹 포함 — workspace-command 는 워크스페이스 생성 시 생성자를 자동으로 admin
     * 그룹에 배정한다.
     *
