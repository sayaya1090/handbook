...
- **선 조사 후 전략 (Explicit Research First):** 구현에 앞서 항상 충분한 조사(Research)를 수행하고, 그 결과를 사용자에게 보고한 뒤 전략을 수립한다.

### 버그 수정 표준 프로세스 (필수 의무 사항)
- **사용자가 버그를 보고하면, 어떤 경우에도 다음 4단계 순서를 반드시, 무조건 지켜야 한다.**
  1. **문제 파악 및 설계**: 버그의 원인을 먼저 분석하고, 어떻게 고칠지 구체적인 설계 방안부터 사용자에게 보고 및 컨펌받는다.
  2. **개발 문서 업데이트**: 설계된 해결책을 바탕으로 관련 문서(`USECASE.md`, `README.md` 등)를 먼저 최신화한다.
  3. **테스트 전략 수립**: 해당 버그 수정 사항을 어떻게 검증할 것인지(E2E, 단위 테스트 등) 명확한 테스트 전략을 세우고 보고하고 검토를 요청한다.
  4. **테스트 코드 작성**: 위 세 단계가 모두 완료되고 사용자가 동의하면, 담당 에이전트에게 위임하여 해당 오류를 검증할 수 있는 테스트 코드를 작성한다. 작성이 완료되면 사용자에게 보고하고 검토를 요청한다.
  5. **코드 수정**: 위 네 단계가 모두 완료되고 사용자의 동의를 득하면, 담당 에이전트에게 위임하여 실제 코드를 수정한다.
...
10. **기존 11개 스코프에 맞지 않는 신규 모듈** → `docs-keeper` 에게 배치 제안 의뢰 (모듈 성격·관련 계약·근접 에이전트 분석) → 메인 Gemini 가 판정: **기존 에이전트 스코프 확장**(선호) vs **신규 에이전트 신설**(복잡도 감당 불가 시). 결정 후 해당 에이전트 정의 파일 + GEMINI.md 도메인 매핑 + `docs/contracts/README.md` 매트릭스 동시 갱신.
...
13. **제너럴리스트(generalist) 서브에이전트 사용 금지**: 다수의 파일에 걸친 단순 반복 작업이나 리팩토링이라 하더라도 `generalist` 에이전트에 위임하는 것을 엄격히 금지한다. 도메인 경계를 넘나드는 작업은 반드시 관련된 **도메인 전문가 에이전트들을 병렬로 호출**하여 각자의 도메인 내에서 처리하도록 위임해야 한다.
