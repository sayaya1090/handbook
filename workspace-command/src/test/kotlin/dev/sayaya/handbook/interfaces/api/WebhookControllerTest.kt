...
class WebhookControllerTest : BehaviorSpec({
    ...
    Given("웹훅 등록 API") {
        ...
        When("POST /workspaces/{workspace}/webhooks를 호출하면") {
            Then("201 Created가 반환된다") {
                client.post()
                    .uri("/workspaces/$workspace/webhooks")
    ...
    Given("웹훅 목록 조회 API") {
        ...
        When("GET /workspaces/{workspace}/webhooks를 호출하면") {
            Then("200 OK가 반환된다") {
                client.get()
                    .uri("/workspaces/$workspace/webhooks")
    ...
    Given("웹훅 삭제 API") {
        ...
        When("DELETE /workspaces/{workspace}/webhooks/{id}를 호출하면") {
            Then("204 No Content가 반환된다") {
                client.delete()
                    .uri("/workspaces/$workspace/webhooks/$webhookId")
                    .exchange()
