...
WITH RECURSIVE
    ...
    version_chain AS (
        ...
        FROM version_chain vc
                 JOIN type t ON t.name = vc.parent
    ...
    -- 4. 버전 정보 필터링
    versions AS (
        SELECT DISTINCT ON (ot.name, ot.effective_at)
            ot.name,
            t.id,
            t.version,
            t.parent,
        ...
        ORDER BY ot.name, ot.effective_at, t.effective_at DESC
    ),
    ...
    inheritance_chain AS (
        ...
        FROM versions v
                 LEFT JOIN attribute att ON att.type = v.id
        UNION ALL
        SELECT
            ic.descendant_type,
            t.version,
        ...
        FROM inheritance_chain ic
                 JOIN type t ON t.name = ic.parent_type
            AND t.effective_at <= ic.effective_at
            AND t.expire_at > ic.effective_at
                 LEFT JOIN attribute att ON att.type = t.id
    ),
