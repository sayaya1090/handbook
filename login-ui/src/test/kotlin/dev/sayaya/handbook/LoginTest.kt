...
internal class LoginTest : GwtTestSpec({
    Given("로그인 화면이 로드됨") {
        Then("콘텐츠 영역(login-content)이 DOM에 존재한다") {
            page.waitForSelector(".login-content") shouldNotBe null
        }

        Then("콘솔 영역(console)이 DOM에 존재한다") {
            page.waitForSelector(".console") shouldNotBe null
        }

        Then("콘솔에 ASCII 아트 웰컴 메시지가 렌더링된다") {
            val console = page.waitForSelector(".console")
        ...
        Then("콘솔에 라인(.line) 요소가 1개 이상 존재한다") {
            page.waitForSelector(".console .line")
        ...
        When("notify 커맨드(error)를 dispatch 하면") {
            ...
            Then("콘솔에 [ERROR] 접두사와 메시지가 출력된다") {
                val text = page.waitForSelector(".console")!!.textContent() ?: ""
