...
| **미구현** | ⚠️ JOIN 모드의 `SubmitButton`은 현재 CREATE 모드만 처리하고, JOIN 요청 API는 미구현. |
...
|------|------|
| **액터** | AI 에이전트 |
| **선행조건** | workspace-ui 모듈 로딩 완료, MutationReceiver 연결 |
...
|----|---|---|---|---|
| UC-W1 (생성) | 워크스페이스 생성 | 전체 | SectionElement, CreateWorkspaceMode, CreateWorkspaceParam, SubmitButton, WorkspaceRepository, WorkspaceApi | WorkspaceTest: 다이얼로그/라디오/Submit 존재, CREATE 선택 + 이름 입력 → Submit 활성화 |
| UC-W2 (참여) | 워크스페이스 참여 요청 | 전체 | SectionElement, CreateWorkspaceMode(JOIN), CreateWorkspaceParam, SubmitButton | WorkspaceTest: JOIN 라디오 선택 → Submit 비활성, 입력 필드 포커스 |
| UC-W3 (에이전트) | 에이전트 워크스페이스 생성 | 전체 | AgentWorkspaceHandler, MutationReceiver, CreateWorkspaceMode, CreateWorkspaceParam, WorkspaceRepository | ❌ 미구현 |
| UC-W4 (단계별) | 에이전트 워크스페이스 생성 (alt) | 전체 | AgentWorkspaceHandler(WS_MODE/WS_INPUT/WS_SUBMIT/WS_CREATE) | ❌ 미구현 |
| UC-W5 (모바일) | 모바일 다이얼로그 전환 | 전체 | ViewportObserver, DialogElement(fullscreen bottom sheet), SectionElement(touch area 48px+), input(full-width) | ❌ 미구현 |
