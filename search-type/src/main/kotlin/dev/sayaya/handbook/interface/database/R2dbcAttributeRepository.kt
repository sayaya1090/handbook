...
class R2dbcAttributeRepository(private val template: R2dbcEntityTemplate) {
    ...
    fun findAllByTypeIds(workspace: UUID, types: List<UUID>): Mono<Map<UUID, List<Attribute>>> = template.select(query(where("type").`in`(types)), R2dbcAttributeEntity::class.java)
    ...
    private fun toDomain(entity: R2dbcAttributeEntity): Attribute = when(entity.attributeType) {
        ...
        AttributeType.Array -> Attribute.Companion.ArrayAttribute (
            ...
            description=entity.description,
            nullable=entity.nullable,
            valueType = entity.valueType ?: throw MissingFieldException("Missing valueType for ArrayAttribute with name: ${entity.name}"),
            inherited = false
        )
        AttributeType.Map -> Attribute.Companion.MapAttribute(
            ...
            description=entity.description,
            nullable=entity.nullable,
            keyType = entity.keyType ?: throw IllegalStateException("Missing keyType for MapAttribute with name: ${entity.name}"),
            valueType = entity.valueType ?: throw IllegalStateException("Missing valueType for MapAttribute with name: ${entity.name}"),
            inherited = false
        )
