...
/**
 * 레이아웃 읽기 전용 비즈니스 로직 (CQRS query-side).
 *
 * **책임:** 워크스페이스별 타입 레이아웃 조회.
 * persist 모듈과의 네임스페이스 충돌을 방지하기 위해 "Search" 접두사를 사용한다.
 *
 * **의존관계:**
 * - [LayoutSearchRepository] — 레이아웃 조회 포트
 */
class LayoutSearchService(private val repo: LayoutSearchRepository) {
