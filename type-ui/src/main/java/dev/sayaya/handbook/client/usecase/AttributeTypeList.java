...
public class AttributeTypeList {
    ...
    @Inject AttributeTypeList(BoxList boxList) {
        boxList.distinct().subscribe(boxes-> {
            var referenceTypes = Arrays.stream(boxes).map(Box::type).filter(Objects::nonNull).map(Type::id);
