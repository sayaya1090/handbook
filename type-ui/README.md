...
## 지능형 정합성 해결 (Intelligent Conflict Resolution)

Type-UI는 복잡한 타입 간 참조 관계를 설정할 때 발생할 수 있는 유효기간 불일치 문제를 지능적으로 해결한다.

1.  **참조 무결성 검증 (UI 1차 방어선)**: 타입 A가 타입 B를 참조할 때, B가 A의 전체 생애 주기 동안 존재하는지 실시간으로 검사한다. (`IntegrityAnalysisService`) Array나 Map 내부 깊숙이 중첩된 Document 참조까지 재귀적으로 모두 추출하여 검증한다.
2.  **안전한 해결책 제안 (Card UI & i18n)**: 정합성이 어긋날 경우, 시스템은 다음과 같은 **2차 오류가 없는 안전한** 보정안만 취합하여 다국어(i18n)가 지원되는 깔끔한 카드(Card) 형태의 다이얼로그로 제시한다. (`ConflictResolutionDialog`)
    *   **소유자 기간 조정**: 타입 A의 시작/종료일을 타입 B가 존재하는 구간으로 축소 (자식 타입이 고립되지 않을 때만 제안).
    *   **참조 대상 기간 확장**: 타입 B의 유효기간을 확장 (다른 버전과 겹치지 않을 때만 제안).
3.  **원클릭 자동 보정**: 사용자가 제안을 승인하면 `ComplexAction`을 통해 기간 조정과 속성 추가를 하나의 트랜잭션으로 처리하여 즉시 정합성을 맞춘다.
4.  **저장 전역 검증 (Pre-save Validation, 최후 방어선)**: AI 에이전트의 다중 명령이나 비정상적인 조작으로 인해 모순된 데이터가 캔버스에 남았을 경우를 대비해, `SaveAction` 실행 직전 무결성을 다시 교차 검증한다. 성능 최적화를 위해 '실제로 변경된 타입'을 중심으로 정방향(자신이 참조) 및 역방향(자신을 참조) 관계만 선택적으로 스캔하며, 위반 발견 시 서버 요청 전 저장을 즉각 차단(Fail-fast)하고 에러 토스트를 표시한다.
