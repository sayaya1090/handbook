...
1.  **참조 무결성 검증**: 타입 A가 타입 B를 참조할 때, B가 A의 전체 생애 주기 동안 존재하는지 실시간으로 검사한다. (`IntegrityAnalysisService`)
2.  **안전한 해결책 제안**: 정합성이 어긋날 경우, 시스템은 다음과 같은 **2차 오류가 없는 안전한** 보정안만 취합하여 사용자에게 제시한다. (`ConflictResolutionDialog`)
