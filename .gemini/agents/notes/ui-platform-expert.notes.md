...
- **테스트 HTML 필수 자산 누락 (2026-04-27)**: 신규 모듈 분리 또는 테스트 HTML 생성 시 RxJS, FontAwesome, sayaya-ui.js 등 필수 라이브러리 임포트를 누락하여 테스트가 런타임에 실패함. 
  - *해결*: 테스트 HTML 작성 시 `type-ui`나 `shell-ui`의 표준 템플릿을 반드시 복사하여 사용할 것.
- **삭제된 테스트 소스 HTML 과잉 삭제 (2026-04-27)**: 빌드 시 생성되는 파일 삭제 시 `src/test/webapp/*.html` 등 수동 관리 필수 자산까지 삭제하여 CI 실패 유발. 해결: .gitignore 정교화 및 소스 복구.
