...
internal class SecurityContextUuidAuditorConfigTest : BehaviorSpec({
    ...
    fun <T : Any> ReactiveAuditorAware<T>.auditor(authentication: Authentication): Mono<T> = currentAuditor
        .contextWrite(ReactiveSecurityContextHolder.withAuthentication(authentication))
    val mockCredentials: Any = mockk()
    Given("인증된 사용자의 principal이 UUID일 때") {
        val userId = UUID.randomUUID()
        val authentication = TestingAuthenticationToken(userId, mockCredentials).apply { isAuthenticated = true }
    ...
    Given("인증된 사용자의 principal이 String 타입의 UUID일 때") {
        val userId = UUID.randomUUID()
        val authentication = TestingAuthenticationToken(userId.toString(), mockCredentials).apply { isAuthenticated = true }
    ...
    Given("사용자가 인증되지 않았을 때") {
        val authentication = TestingAuthenticationToken(UUID.randomUUID(), mockCredentials) // isAuthenticated is false
    ...
    Given("인증된 사용자의 principal이 예상치 못한 타입일 때") {
        ...
        val authentication = TestingAuthenticationToken(principal, mockCredentials).apply { isAuthenticated = true }
