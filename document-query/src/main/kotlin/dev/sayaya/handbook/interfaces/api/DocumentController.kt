...
class DocumentController(private val svc: DocumentSearchService) {
    @GetMapping("/workspaces/{workspace}/documents", produces = ["application/vnd.sayaya.handbook.v1+json"])
    fun search(@PathVariable workspace: UUID, query: Search): Mono<Page<Document>> = svc.search(workspace, query)

    @GetMapping("/workspaces/{workspace}/{type}/{serial}", produces = ["application/vnd.sayaya.handbook.v1+json"])
    fun find(
    ...
    @GetMapping("/workspaces/{workspace}/documents/search", produces = ["application/vnd.sayaya.handbook.v1+json"])
    fun fullTextSearch(
        @PathVariable workspace: UUID,
    ...
    @GetMapping("/workspaces/{workspace}/{type}/{serial}/history", produces = ["application/vnd.sayaya.handbook.v1+json"])
    fun history(
        @PathVariable workspace: UUID,
        @PathVariable type: String,
        @PathVariable serial: String,
    ...
    @GetMapping("/workspaces/{workspace}/{type}/{serial}/diff", produces = ["application/json"])
