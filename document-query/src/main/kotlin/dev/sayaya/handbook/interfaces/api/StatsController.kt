 ...
 *
 * **엔드포인트:**
 * - `GET /workspaces/{workspace}/stats` — 워크스페이스 집계 통계
 * - `GET /workspaces/{workspace}/stats/timeline?from=&to=&interval=` — 시간별 문서 통계
 * - `GET /workspaces/{workspace}/stats/distribution` — 타입별 문서 분포
 * - `GET /workspaces/{workspace}/quality-issues` — 품질 이슈 목록
 * - `GET /workspaces/{workspace}/agent-activity` — 에이전트 활동 목록
 */
@RestController
class StatsController(private val svc: StatsService) {
     ...
     */
    @GetMapping("/workspaces/{workspace}/stats", produces = ["application/json"])
     ...
     * @param workspace 워크스페이스 ID
     */
    @GetMapping("/workspaces/{workspace}/quality-issues", produces = ["application/json"])
    @ResponseStatus(HttpStatus.OK)
     ...
     * @param workspace 워크스페이스 ID
     */
    @GetMapping("/workspaces/{workspace}/agent-activity", produces = ["application/json"])
    @ResponseStatus(HttpStatus.OK)
     ...
     */
    @GetMapping("/workspaces/{workspace}/stats/timeline", produces = ["application/json"])
    @ResponseStatus(HttpStatus.OK)
    fun timeline(
     ...
     * @param workspace 워크스페이스 ID
     */
    @GetMapping("/workspaces/{workspace}/stats/distribution", produces = ["application/json"])
    @ResponseStatus(HttpStatus.OK)
    fun distribution(
        @PathVariable workspace: UUID,
