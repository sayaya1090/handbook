...
class ExportControllerTest : BehaviorSpec({
    ...
    Given("JSON 형식 내보내기 API") {
        ...
        When("GET /workspaces/{id}/documents/export?format=json을 호출하면") {
            Then("200 OK + JSON 응답이 반환된다") {
                client.get()
                    .uri("/workspaces/$workspace/documents/export?format=json&page=0&limit=10")
                    .exchange()
                    .expectStatus().isOk
    ...
    Given("CSV 형식 내보내기 API") {
        ...
        When("GET /workspaces/{id}/documents/export?format=csv를 호출하면") {
            Then("200 OK + CSV 응답이 반환된다") {
                client.get()
                    .uri("/workspaces/$workspace/documents/export?format=csv&page=0&limit=10")
                    .exchange()
                    .expectStatus().isOk
    ...
    Given("지원하지 않는 형식 요청") {
        When("format=xml을 요청하면") {
            Then("400 Bad Request가 반환된다") {
                client.get()
                    .uri("/workspaces/$workspace/documents/export?format=xml&page=0&limit=10")
                    .exchange()
