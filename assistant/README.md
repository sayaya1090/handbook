...
- **협업 모드** (UC-A1~A4): 사용자 요청 기반. 에이전트 커맨드가 프론트엔드에서 시각적으로 실행되어 화면 조작 UX 제공.
...
| POST | `/assistant/respond` | `application/vnd.sayaya.handbook.v1+json` | 사용자 응답 전달 (await_confirm 후) |
| POST | `/assistant/abort` | - | 현재 실행 취소 |

## 구조

```
├── domain/          CommandType, AgentCommand, ExecutionStep, ExecutionPlan, QualityIssue
...
3. `PlanExecutor`가 각 단계를 순서대로 실행하여 `AgentCommand`를 생성한다.
...
- **실행 방식**: 사용자 자연어 요청, 주기적 스케줄, 또는 DOCUMENT_CREATED 이벤트 트리거
...
| AtomicReference로 실행 관리 | 동시성 안전한 abort 지원 |
