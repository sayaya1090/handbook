...
sequenceDiagram
    ...
    loop 각 그룹 (group 순서대로)
        ...
        loop 각 step 완료 시
            ...
            Pub->>K: AGENT_COMMAND 이벤트 발행
            K->>EB: 이벤트 수신
            EB-->>User: SSE /workspaces/{id}/messages (type: navigate/highlight/...)
        end
    end
    ...
    Pub->>K: AGENT_COMMAND 이벤트 발행
    K->>EB: 이벤트 수신
    EB-->>User: SSE /workspaces/{id}/messages (type: complete + artifact)
```
...
sequenceDiagram
    ...
    QM->>GW: GET /workspaces/{id}/documents (document-query API 경유)
    GW-->>QM: 문서 목록
    QM->>QM: 결측치 / 중복 / 이상값 분석
    ...
    loop 이슈별 알림
        ...
        Pub->>K: AGENT_COMMAND 이벤트 발행
        K->>EB: 이벤트 수신
        EB-->>User: SSE /workspaces/{id}/messages (type: notify)
    end
...
sequenceDiagram
    ...
    K->>EB: 이벤트 수신
    EB-->>C1: SSE /workspaces/ws-1/messages (AGENT_COMMAND)
    EB-->>C2: SSE /workspaces/ws-1/messages (AGENT_COMMAND)
