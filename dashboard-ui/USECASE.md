...
| **요구사항** | 6.2 대시보드 API 통합 — API URL을 워크스페이스 기반으로 구성 (GET /workspaces/{workspaceId}/quality-issues, GET /workspaces/{workspaceId}/agent-activity) |
...
| UC-DB4 (실행 상태) | 실시간 에이전트 실행 상태 위젯 | DashboardApi, ActiveExecutionsWidget, ActiveExecutionList, SSE AGENT_COMMAND | DashboardAgentWidgetsTest: 활성 실행 위젯/헤더/리스트 존재, 실행 2건, 의도 텍스트, 진행률 바/fill/텍스트(슬래시 형식), 상태 RUNNING 검증 |
