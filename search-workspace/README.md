...
| 모듈 | 역할 | 주 메서드 |
|------|------|----------|
| `persist-workspace` | 쓰기 (CUD) + 웹훅 등록/삭제 | POST/PUT/DELETE |
| `search-workspace` | 읽기 + 메뉴 공급 | GET `/menus` |
| `workspace` | 도메인 모델 + 유스케이스 | - |

역할 분리는 다른 도메인(type, document)이 이미 따르는 패턴(`search-type`/`persist-type`,
`search-document`/`persist-document`)과 일관화를 위한 것이다.
...
gateway 의 `search-workspace` 라우트 (Method=GET) 가 이 서비스로 포워딩한다.
...
```bash
./gradlew :search-workspace:bootRun    # 기본 8080
./gradlew :search-workspace:test       # 단위 테스트
```

프로덕션 port 는 gateway ConfigMap 의 `services` 목록에서
`service-search-workspace:8080` 으로 지정된다.
